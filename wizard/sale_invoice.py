...
class SaleInvoice(models.TransientModel):
    ...
    def _get_advance_payment_method(self):
        if self._count() == 1:
            ...
            if order.order_line.filtered(lambda dp: dp.is_downpayment) and order.invoice_ids.filtered(
                    lambda invoice: invoice.state != 'cancel') or order.order_line.filtered(lambda l: l.qty_to_invoice < 0):
                return 'all'
            else:
                return 'delivered'
        return 'all'
